...
export default function Page() {
  ...
  const { isLoading, data: categoriesData } = useGetCategoriesQuery();

  const categoryName = (() => {
    if (pathname.endsWith("/partners")) {
      return "Для партнеров";
    }
  })();
  ...
  }, [isAuthenticated, customer, isLoading, toggleModal]);

  // Получаем ID категорий для партнеров
  const partnerCategories = React.useMemo(() => {
    if (!categoriesData?.data) return [];
    
    return categoriesData.data
      .filter(category => 
        category.name?.toLowerCase().includes('партнер') ||
        category.name?.toLowerCase().includes('опт') ||
        category.forPartners === true
      )
      .map(category => ({
        id: category.id,
        name: category.name || category.attributes?.name || 'Без названия'
      }));
  }, [categoriesData]);

  // Если проверяем доступ или нет доступа - показываем сообщение
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Loader />
        <p className="ml-3">Загрузка...</p>
      </div>
    );
  }

  // Если пользователь не авторизован или не имеет нужный статус
  if (!isAuthenticated || !customer || customer.type !== "Оптовый покупатель") {
    return (
        ...
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <p className="text-gray-600 mb-6">
            {!isAuthenticated || !customer 
              ? "Для доступа к разделу партнеров необходимо авторизоваться"
              : "Доступ к разделу партнеров только для оптовых покупателей"
            }
          </p>
          <button
            onClick={() => toggleModal("modals_auth")}
            className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors"
          >
            {!isAuthenticated || !customer ? "Войти в аккаунт" : "Войти как оптовый покупатель"}
          </button>
        </div>
      </div>
    );
